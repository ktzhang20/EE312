...
int minRec1(int x[], int n) {
    if(n == 1)
      return x[0];
    int min = minRec1(x, n - 1);
    return min < x[n-1] ? min : x[n-1];
}
...
double sqrtIt(double x, double low_guess, double high_guess) {
  ...
  double i = .00000000000001;
  while((high_guess-low_guess)/avg > i){
    avg = (low_guess+high_guess)/2;
    if((high_guess - low_guess)/ avg < i){
      return avg;
    }
    if(avg*avg > x){
      high_guess = avg;
...
double sqrtRec(double x, double low_guess, double high_guess) {
  double avg = (high_guess+low_guess)/2;
  double i = .00000000000001;
    if((high_guess - low_guess) /avg < i)
      return avg;
    if(avg*avg > x)
...
int strCompare2(char* str1, char* str2) {
  if((str1[1] == 0 || whatLetter(str1[1]) == -1)  && (str2[1] == 0 || whatLetter(str2[1]))){
    return 0;
  }
  if(whatLetter(str1[0]) == -1 && whatLetter(str2[0]) == -1){
...
/*
 * return the value of the direction that is one turn to the high_guess
 */
...
/*
 * return the value of the direction that is one turn to the low_guess
 */
int turnlow_guess(int dir) {
    return (dir + 3) % 4;
}
 ...
 * Your starting point is at row and col. (params to this function)
 * Your exit point is somewhere in the last row (which is: MATRIX_SIZE - 1)
 * The maze can be solved by following the high_guess hand wall.  In fact, there
 ...
 *       and then turn high_guess.  
 *    case 2: you cannot move in the current direction
 *       If there's a wall directly in front of you, then to follow the high_guess
 *       hand wall, you'd need to turn low_guess (placing your hand on the wall that
 *       used to be directly in front of you).  So, if you discover this case
 *       then turn low_guess.  Don't move, just turn low_guess.
 ...
 * take a step, and then turn high_guess (case 1).  This causes you to walk
 ...
 * forward and therefore (case 2) you'll turn low_guess.  On the third iteration
...
void solveMazeIt(int row, int col) {
    int dir = 2; // 0 is up, 1 is high_guess, 2 is down, 3 is low_guess.
...
Martian change(int cents, int nick_val, int dodek_val) {
  if(cents < 0){
    return {INT16_MAX,0,0};
  }
  if(cents == 0){
    return {0,0,0};
  }

  Martian m12 = change(cents - dodek_val, nick_val, dodek_val);
  Martian m5 = change(cents - nick_val, nick_val, dodek_val);
  Martian m1 = change(cents - 1, nick_val, dodek_val);

  if(getSum(m12) < getSum(m5) && getSum(m12) < getSum(m1)){
    m12.dodeks += 1;
    return m12;
  }
  if(getSum(m5) < getSum(m12) && getSum(m5) < getSum(m1)){
    m5.nicks ++;
    return m5;
  }
  m1.pennies ++;
...
Martian changeIt(int cents) {
    return {0,0,0}; // delete this line, it's broken. Then write the function properly!
}
